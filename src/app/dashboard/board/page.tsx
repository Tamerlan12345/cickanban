...
export default function BoardPage() {
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    const matchesCycle    = filterCycle    === 'ALL' || task.cycleId === filterCycle;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '30px' }}>
      ...
      >
        ...
        {cycles.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Спринт:</span>
            <select
              value={filterCycle}
            ...
            >
              <option value="ALL" style={{ background: '#0b081a' }}>Все</option>
              {cycles.map((c) => (
                <option key={c.id} value={c.id} style={{ background: '#0b081a' }}>{c.name}</option>
              ))}
            </select>
          </div>
        )}
        ...
        {/* Reset filter button */}
        {(searchQuery || filterPriority !== 'ALL' || filterAssignee !== 'ALL' || filterCycle !== 'ALL') && (
          <button
            onClick={() => {
              setSearchQuery('');
